...
class UserInfo(forms.Form):
    ...
    def clean_homepage(self):
        ...
        if not homepage.startswith('https'):
            raise ValidationError('httpsではじめてください')
        return homepage
    ...
    def clean(self):
        ...
        if mail != verify_mail:
            self.add_error('mail', 'メールアドレスが一致しません')
...
class PostModelForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if post:
            raise ValidationError('そのタイトルとメモは存在します')
        return cleaned_data
...
MemoModelFormSet = forms.modelformset_factory(
    Memo, form=MemoModelForm, extra=3, max_num=5, validate_max=True,
)
